...
var Pagination = function (el, options) {
	...
	var $container = $(el),
		...
		pageRange = options.pageRange || [15, 30, 45, -1], //-1 (All)
		pageSize = options.pageSize || pageRange[0],

		allPagesLabel = options.labels.all || 'All',
		firstPageLabel = options.labels.first || 'Primeiro',
		lastPageLabel = options.labels.last || 'Último',
	...
	getRenderedPageRange = function () {
		var result = '<select class="page-range hidden">';
	...
	getRenderedPageNumbers = function () {
		...
		result = '<li data-page="' + FIRST_PAGE_NUMBER + '"><a style="cursor: pointer"><span aria-hidden="true">' + firstPageLabel + '</span></a></li>' +
				 '<li data-page="' + PREVIOUS_PAGE_NUMBER + '"><a style="cursor: pointer"><span aria-hidden="true">' + previousPageLabel + '</span></a></li>';

		$.each(pageNumbers, function (i, p) {
			result += '<li data-page="' + p + '" ' + (p === currentPage ? 'class="active"' : '') + '><a style="cursor: pointer">' + p + '</a></li>';
		});

		result += '<li data-page="' + NEXT_PAGE_NUMBER + '"><a style="cursor: pointer"><span aria-hidden="true">' + nextPageLabel + '</span></a></li>' +
				  '<li data-page="' + LAST_PAGE_NUMBER + '"><a style="cursor: pointer"><span aria-hidden="true">' + lastPageLabel + '</span></a></li>';
